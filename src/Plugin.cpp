...
void Plugin::registerTypes(QQmlEngine *engine, const char *uri)
{
    ...
    qmlRegisterSingletonType<AudioTagHelper>(uri, 1, 0, "AudioTagHelper", [](QQmlEngine *engine, QJSEngine *scriptEngine) -> QObject * {
        Q_UNUSED(engine)
        Q_UNUSED(scriptEngine)
        AudioTagHelper *audioTagHelper = AudioTagHelper::instance();
        QQmlEngine::setObjectOwnership(audioTagHelper, QQmlEngine::CppOwnership);
        return audioTagHelper;
    });
    qmlRegisterSingletonType<SndLibrary>(uri, 1, 0, "SndLibrary", [](QQmlEngine *engine, QJSEngine *scriptEngine) -> QObject * {
        Q_UNUSED(engine)
        Q_UNUSED(scriptEngine)
...
QList<QList<JackPassthrough *>> Plugin::sketchFxPassthroughClients() const
{
    return m_sketchFxPassthroughClients;
}
