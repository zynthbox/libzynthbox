...
QString ClipAudioSource::slicesToString() const
{
  ...
  for (int sliceIndex = 0; sliceIndex < d->sliceCount; ++sliceIndex) {
    ...
    for (int subvoiceIndex = 0; subvoiceIndex < slice->subvoiceCount(); ++subvoiceIndex) {
      ...
      subvoiceObject.insert("gain", subvoice->gainDb());
...
void ClipAudioSource::stringToSlices(const QString& data)
{
  ...
  if (settingsObject.contains("settings")) {
    ...
    for (int sliceIndex = 0; sliceIndex < qMin(d->sliceCount, d->sliceSettingsActual.count()); ++sliceIndex) {
      ...
      slice->setADSRAttack(sliceObject.value("adsrAttack", 0.0).toFloat());
      slice->setADSRDecay(sliceObject.value("adsrDecay", 0.0).toFloat());
      slice->setADSRSustain(sliceObject.value("adsrSustain", 1.0).toFloat());
      slice->setADSRRelease(sliceObject.value("adsrRelease", 0.0).toFloat());
      slice->setGrainInterval(sliceObject.value("grainInterval", 10).toFloat());
      slice->setGrainIntervalAdditional(sliceObject.value("grainIntervalAdditional", 10).toFloat());
      slice->setGrainPanMaximum(sliceObject.value("grainPanMaximum", 1.0).toFloat());
      slice->setGrainPanMinimum(sliceObject.value("grainPanMinimum", -1.0).toFloat());
      slice->setGrainPitchMaximum1(sliceObject.value("grainPitchMaximum1", 1.0).toFloat());
      slice->setGrainPitchMaximum2(sliceObject.value("grainPitchMaximum2", 1.0).toFloat());
      slice->setGrainPitchMinimum1(sliceObject.value("grainPitchMinimum1", 1.0).toFloat());
      slice->setGrainPitchMinimum2(sliceObject.value("grainPitchMinimum2", 1.0).toFloat());
      slice->setGrainPitchPriority(sliceObject.value("grainPitchPriority", 0.5).toFloat());
      slice->setGrainPosition(sliceObject.value("grainPosition", 0.0).toFloat());
      slice->setGrainScan(sliceObject.value("grainScan", 0.0).toFloat());
      slice->setGrainSize(sliceObject.value("grainSize", 100).toFloat());
      slice->setGrainSizeAdditional(sliceObject.value("grainSizeAdditional", 50).toFloat());
      slice->setGrainSpray(sliceObject.value("grainSpray", 1.0).toFloat());
      slice->setGrainSustain(sliceObject.value("grainSustain", 0.3).toFloat());
      slice->setGrainTilt(sliceObject.value("grainTilt", 0.5f).toFloat());
      ...
      slice->setStartPositionSamples(sliceObject.value("startPositionSamples", 0).toInt());
      slice->setLengthSamples(sliceObject.value("lengthSamples", getDurationSamples()).toInt());
      slice->setLoopDeltaSamples(sliceObject.value("loopDeltaSamples", 0).toInt());
      slice->setLoopDelta2Samples(sliceObject.value("loopDelta2Samples", 0).toInt());
      ...
      for (int subvoiceIndex = 0; subvoiceIndex < slice->subvoiceCount(); ++subvoiceIndex) {
        ...
        ClipAudioSourceSubvoiceSettings *subvoice = slice->subvoiceSettingsActual().at(subvoiceIndex);
        subvoice->setPan(subvoiceObject.value("pan", 0.0f).toFloat());
        subvoice->setPitch(subvoiceObject.value("pitch", 0.0f).toFloat());
        subvoice->setGainDb(subvoiceObject.value("gain", 0.0f).toFloat());
      }
    }
