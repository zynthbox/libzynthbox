...
struct alignas(64) TimerCommand {
    ...
    enum Operation {
        ...
        SetSongPositionOperation = 101, ///@< Sets the song position explicitly to the value in parameter (a number of MIDI Beats) - only informational, the actual counting and offset handling is done using RegisterMidiClockSyncOperation
        RegisterMidiClockSyncOperation = 102, ///@< INTERNAL - Used to forward MIDI Clock message information from the transport manager's ingest process to SyncTimer (parameter holding the jack frame for the event, and parameter2 holding the specific clock operation (that is, the first byte of the midi message), and for clock events specifically, parameter3 holds the clock source's ppqn, and for song position pointer calls parameter3 holds the new song position)
